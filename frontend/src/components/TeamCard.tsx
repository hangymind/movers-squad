import { Clock3, Crown, LogIn, LogOut, MessageSquareText, Users, XCircle } from 'lucide-react'
  ...
  return (
    <article className="team-card">
      <div className="team-card-top">
        <div className="game-mark"><span>{team.gameName.charAt(0).toUpperCase()}</span></div>
        <div className="team-title-wrap">
          <div className="team-title-row"><h3>{team.gameName}</h3>{team.isFull ? <span className="status status-full">已满员</span> : <span className="status">招募中</span>}</div>
          <span className="created-time"><Clock3 size={14} />{formatCreatedAt(team.createdAt)}</span>
        </div>
      </div>

      <div className={`team-note ${team.note ? '' : 'team-note-empty'}`}><MessageSquareText size={17} /><p>{team.note ?? '队长暂未添加备注'}</p></div>
      <div className="team-rules"><span>等级 ≥ {team.minLevel ?? 1}</span>{(team.excludedFlorrIds ?? []).length > 0 && <span>排除 {(team.excludedFlorrIds ?? []).length} 个 ID</span>}</div>

      <div className="captain-row">
        <Avatar user={team.owner} size="sm" />
        <span><strong>{team.owner.florrId}</strong><small><Crown size={12} />队长 · 等级 {team.owner.level ?? 1}</small></span>
      </div>
      ...
      <div className="member-section">
        ...
        <div className="member-list">
          {team.members.map((member) => <span className="member-item" key={member.id}><Avatar user={member} /><small>{member.florrId}</small></span>)}
