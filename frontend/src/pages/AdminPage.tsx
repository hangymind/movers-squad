...
export function AdminPage() {
  ...
  const closeTeam = async (team: Team) => {
    setBusyId(team.id)
    try { const { data } = await api.post<{ data: Team }>(`/admin/teams/${team.id}/close`); setTeams((current) => current.map((item) => item.id === team.id ? data.data : item)) }
    catch (requestError) { setError(getErrorMessage(requestError)) }
    finally { setBusyId(null) }
  }
  const confirmDeleteTeam = async () => {
    ...
    try { await api.delete(`/admin/teams/${deleteTeam.id}`); setTeams((current) => current.filter((item) => item.id !== deleteTeam.id)); setDeleteTeam(null) }
    catch (requestError) { setError(getErrorMessage(requestError)) }
    finally { setBusyId(null) }
  }
  ...
  return <div className="admin-page">
    ...
    {deleteTeam && <div className="modal-backdrop"><section className="modal admin-delete-dialog" role="alertdialog" aria-modal="true"><div className="modal-header"><h2>删除组队</h2></div><p>确定永久删除 <strong>{deleteTeam.gameName}</strong>？成员关系、聊天记录和语音房间都会被清理。</p><div className="modal-actions"><button type="button" className="button-secondary" onClick={() => setDeleteTeam(null)}>取消</button><button type="button" className="button-danger" onClick={() => void confirmDeleteTeam()}><Trash2 size={16} />确认删除</button></div></section></div>}
