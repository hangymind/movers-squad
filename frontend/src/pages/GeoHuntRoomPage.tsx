...
import { Copy, Crown, LogOut, Play, UsersRound } from 'lucide-react'
...
export function GeoHuntRoomPage({ user, onLogout }: Props) {
  ...
  const roomTitle = room ? (room.name ?? `私人对局 ${room.code}`) : null
  ...
  return <div className="geo-page">
    ...
    <main className="geo-room-shell">
      <section className="geo-room-heading"><div><span className="eyebrow">CUSTOM MAP DUEL</span><h1>{room.name ?? '私人对局'}</h1><p>{room.mode === 'admin_public' ? '管理员公开多人房' : '不公开私人房'} · {room.playerCount}/{room.maxPlayers} 人</p></div><button className="geo-room-code" type="button" onClick={() => void copyCode()} title="复制房间码"><span>房间码</span><strong>{room.code}</strong><Copy size={17} />{copied && <small>已复制</small>}</button></section>
